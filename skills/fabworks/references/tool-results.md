...
- Bent parts include `bends.items`, one entry per bend in model order: `bend`, `angle_deg`, `inside_radius`, `tooling_radius`, `flange_length`, `min_flange_length`, `closest_cutout`, `distortion_zone`, `length`, `max_length`, `max_angle_deg` (inches and degrees), and `issues` with a `code` such as `flange_too_short` or `cutout_in_distortion_zone`. Use them to say which bend fails and by how much, for example "bend 2 flange is 0.591 in; 0.187 in stock needs 0.708 in".
